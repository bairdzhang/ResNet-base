...
from tensorpack.dataflow import AugmentImageComponent, BatchData, LocallyShuffleData, MultiThreadMapData, PrefetchDataZMQ, MapDataComponent, PrefetchData, dataset, imgaug
from tensorpack.dataflow.serialize import LMDBSerializer
from tensorpack.dataflow.format import LMDBDataPoint
...
def get_imagenet_dataflow(datadir,
    ...
    if isTrain:
        ds = LMDBSerializer.load(
            os.path.join(datadir, 'train_lmdb/train.lmdb'), shuffle=False)
        ...
        ds = MultiThreadMapData(
            ds, parallel, mapf, buffer_size=2000, strict=True)
...
def get_imagenet_tfdata(datadir, name, batch_size, mapper=None, parallel=None):
    ...
    def get_imglist(dir, name):
        ...
        imglist = meta.get_image_list(
            name, dataset.ILSVRCMeta.guess_dir_structure(dir))
    ...
    labels = tf.constant([k[1] for k in imglist],
                         dtype=tf.int32,
                         name='labels')
...
def fbresnet_mapper(isTrain):
    ...
    def resize_shortest_edge(image, image_shape, size):
        ...
        shape = tf.cond(
            w_greater, lambda: tf.cast([shape[0] / shape[
                1] * size, size], tf.int32), lambda: tf.cast(
                    [size, shape[1] / shape[0] * size], tf.int32))
    ...
    def center_crop(image, size):
        ...
        image = tf.slice(image, [offset_height, offset_width, 0],
                         [size, size, -1])
        return image
    ...
    def validation_mapper(byte):
        image = tf.image.decode_jpeg(tf.reshape(byte, shape=[]), 3, **JPEG_OPT)
    ...
    def training_mapper(byte):
        ...
        is_bad = tf.reduce_sum(
            tf.cast(tf.equal(bbox_size, jpeg_shape), tf.int32)) >= 2
...
class ImageNetModel(ModelDesc):
    ...
    def inputs(self):
        return [
            tf.placeholder(self.image_dtype,
                           [None, self.image_shape, self.image_shape, 3],
                           'input'),
